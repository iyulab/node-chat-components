...
export class UDataElement extends UElement {
   ...
   * @param attrs - 추가 속성 객체 (선택적, `HtmlBuilder.build` 규칙 적용)
  ...
  public static buildHTML(
    ...
    if (!tag) {
      console.error(`Custom element for ${this.name} is not defined.`);
      return '';
    }
  ...
  /**
   * 오류 발생 시 콘솔에만 조용히 기록합니다. 에러 UI를 띄우지 않으므로 채팅 화면이
   * 갑작스러운 에러 카드로 어수선해지지 않습니다.
  ...
  protected async error(error: unknown): Promise<void> {
    console.error(`failed to load data for [${this.localName}]`, error);
  }
}
