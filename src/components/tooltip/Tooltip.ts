...
import { computePosition, offset, shift, flip, arrow, autoPlacement } from '@floating-ui/dom';
...
export class Tooltip extends BaseElement {
  ...
  @query('.arrow') arrowEl!: HTMLElement;
  ...
  /** 툴팁에 화살표를 표시할지 여부를 설정합니다. 기본값은 false입니다. */
  @property({ type: Boolean }) arrow: boolean = false;

  /** 툴팁이 상호작용 가능한지 여부를 설정합니다. 기본값은 true입니다. */
  ...
  render() {
    return html`
      ...
      <div class="arrow" ?hidden=${!this.arrow}></div>
    `;
  }
  ...
  public show = async () => {
    ...
    if (!this.trigger || this.isEmpty) return;
