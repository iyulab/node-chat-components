...
import {
  ...
  isPrimitive,
  getValueByPath,
  toggleNode, expand, filter, highlight, resetFilter
} from './JsonViewer.utils';
import type { JSONArray, JSONObject, JSONValue, JsonViewerState, Primitive } from './JsonViewer.types';
...
export class JsonViewer extends BaseElement {
  ...
  static customRenderer(value: Primitive): TemplateResult | Node | string {
  ...
  *search(criteria: string | RegExp) {
  ...
  renderValue(value: JSONValue, path = '') {
    if (isPrimitive(value)) {
      return this.renderPrimitive(value as Primitive, path);
    }
